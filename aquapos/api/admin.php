...
switch ($action) {
    ...
    // ── Get all refills ───────────────────────────────
    case 'get_refills':
        $result = $conn->query("SELECT * FROM refills ORDER BY sort_order, id");
        echo json_encode(['success' => true, 'data' => $result->fetch_all(MYSQLI_ASSOC)]);
        break;

    // ── Update refill price ───────────────────────────
    case 'update_refill_price':
        $id    = intval($_POST['id']    ?? 0);
        $price = floatval($_POST['price'] ?? 0);
        if (!$id || $price < 0) { echo json_encode(['success' => false, 'message' => 'Invalid data.']); break; }
        $stmt = safePrepare($conn, "UPDATE refills SET price=?, updated_at=NOW() WHERE id=?");
        $stmt->bind_param("di", $price, $id);
        $stmt->execute();
        echo json_encode(['success' => true]);
        break;

    // ── Update refill (full edit) ─────────────────────
    case 'update_refill':
        $id   = intval($_POST['id']   ?? 0);
        $name = trim($_POST['name']   ?? '');
        $desc = trim($_POST['description'] ?? '');
        $price= floatval($_POST['price']   ?? 0);
        if (!$id || !$name) { echo json_encode(['success' => false, 'message' => 'Name required.']); break; }
        $stmt = safePrepare($conn, "UPDATE refills SET name=?, description=?, price=?, updated_at=NOW() WHERE id=?");
        $stmt->bind_param("ssdi", $name, $desc, $price, $id);
        $stmt->execute();
        echo json_encode(['success' => true, 'message' => 'Refill updated.']);
        break;
    ...
    case 'add_refill':
        ...
        $stmt = safePrepare($conn, "INSERT INTO refills (name, description, price) VALUES (?, ?, ?)");
        $stmt->bind_param("ssd", $name, $desc, $price);
        $stmt->execute();
        echo json_encode(['success' => true, 'message' => 'Refill added.']);
        break;

    // ── Toggle refill status ──────────────────────────
    case 'toggle_refill':
        $id = intval($_POST['id'] ?? 0);
        $conn->query("UPDATE refills SET status = IF(status='active','inactive','active') WHERE id=$id");
        $r = $conn->query("SELECT status FROM refills WHERE id=$id")->fetch_assoc();
        echo json_encode(['success' => true, 'status' => $r['status']]);
        break;

    // ── Refill price version (for POS sync) ───────────
    case 'refill_version':
    ...
    // ── Get all refills ───────────────────────────────
    case 'get_refills':
        $result = $conn->query("SELECT * FROM refills ORDER BY sort_order ASC, id ASC");
        echo json_encode(['success' => true, 'data' => $result->fetch_all(MYSQLI_ASSOC)]);
        break;
    ...
    // ── Update refill (name, desc, price, status) ─────
    case 'update_refill':
        $id     = intval($_POST['id']          ?? 0);
        $name   = trim($_POST['name']          ?? '');
        $desc   = trim($_POST['description']   ?? '');
        ...
        if (!$id || !$name) { echo json_encode(['success' => false, 'message' => 'Name required.']); break; }
        $stmt = safePrepare($conn, "UPDATE refills SET name=?, description=?, price=?, status=?, updated_at=NOW() WHERE id=?");
        $stmt->bind_param("ssdsi", $name, $desc, $price, $status, $id);
        $stmt->execute();
        $stmt->close();
        echo json_encode(['success' => true, 'message' => 'Refill updated.']);
        break;

    // ── Add new refill option ─────────────────────────
    case 'add_refill':
        $name  = trim($_POST['name']        ?? '');
        $desc  = trim($_POST['description'] ?? '');
        $price = floatval($_POST['price']   ?? 0);
        ...
        $stmt->execute();
        $stmt->close();
        echo json_encode(['success' => true, 'message' => 'Refill option added.']);
        break;
    ...
    // ── Refill price version (for POS real-time sync) ─
    case 'refill_version':
        $r = $conn->query("SELECT MAX(updated_at) as ts FROM refills");
        echo json_encode(['success' => true, 'ts' => $r->fetch_assoc()['ts']]);
        break;
